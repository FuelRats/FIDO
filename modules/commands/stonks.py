...
async def crypto_compare(bot: fido, channel: str, sender: str, args: List[str]):
    ...
    if len(args) == 1:
        ...
        price = cryptocompare.get_price(args[0].upper(), 'USD')[args[0].upper()]['USD']
        if price:
        ...
        price = cryptocompare.get_price(args[0].upper(), args[1].upper())[args[0].upper()][args[1].upper()]
        if price:
...
async def crypto_value(bot: fido, channel: str, sender: str, args: List[str]):
    ...
    if channel not in ['#stonks']:
        ...
        return
    if len(args) == 2:
        price = cryptocompare.get_price(args[1].upper(), 'USD')[args[1].upper()]['USD']
        if price:
        ...
        price = cryptocompare.get_price(args[1].upper(), args[2].upper())[args[1].upper()][args[2].upper()]
        if price:
            value = price * float(args[0])
